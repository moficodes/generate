...
func BenchmarkWriteFile10_000_4workers(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		writeToFile(ctx, filename, 4, 2_500)
	}
}
...
func BenchmarkWriteFile10_000_50workers(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		writeToFile(ctx, filename, 50, 200)
	}
}

func BenchmarkWriteFile1_000_000_seq(b *testing.B) {
	ctx := context.Background()
	for i := 0; i < b.N; i++ {
		tmpfile, err := os.CreateTemp("", "")
		if err != nil {
		...
		writeToFile(ctx, filename, 1, 1_000_000)
	}
}

func BenchmarkWriteFile1_000_000_4workers(b *testing.B) {
	ctx := context.Background()
	for i := 0; i < b.N; i++ {
		tmpfile, err := os.CreateTemp("", "")
		if err != nil {
		...
		writeToFile(ctx, filename, 4, 250_000)
	}
}
